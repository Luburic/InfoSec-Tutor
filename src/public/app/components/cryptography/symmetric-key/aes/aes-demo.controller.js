(function() {
	...
	angular
		...
		.controller('AESDemoController', AESDemoController);

  AESDemoController.$inject = ['itConstants', '$http'];
	function AESDemoController(itConstants, $http) {
		var vm = this;
    ...
    function submit() {
      if(vm.submission.key) {
        if(vm.submission.plaintext) {
          ...
          $http.get(itConstants.api + 'demo/aes?text=' + vm.submission.plaintext + '&key=' + vm.submission.key + '&mode=encryption').then(function(res) {
          ...
          vm.enableSubmission = false;
          $http.get(itConstants.api + 'demo/aes?text=' + vm.submission.ciphertext + '&key=' + vm.submission.key + '&mode=decryption').then(function(res) {
